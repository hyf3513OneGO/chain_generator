...
def process_relaxation_sync(msg: dict, config: ChainRelaxConfig, worker_id: str):
    ...
    relaxation_time = time.time() - start_time
    ...
    print(f"[时间统计] 分子松弛: {relaxation_time:.3f}s 保存松弛分子: {save_relaxed_time:.3f}s, 保存初始分子: {save_init_time:.3f}s, 保存PSMILES: {save_psmiles_time:.3f}s, 移除氢原子: {remove_hs_time:.3f}s")
