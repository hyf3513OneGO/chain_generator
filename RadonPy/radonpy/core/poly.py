...
def terminate_mols(poly, mol1, mol2=None, bond_length=1.5, dihedral=np.pi,
                   random_rot=False, dih_type='monomer', terminate_tail=True):
    """
    poly.terminate_mols
    ...
    Args:
        ...
        mol1: terminated substitute at head (RDKit Mol object)

    Optional args:
        mol2: terminated substitute at tail (RDKit Mol object). Ignored if terminate_tail=False
        bond_length: Bond length of connecting bond (float, angstrom)
        random_rot: Randomly rotate dihedral angle around connecting bond (bool)
        dihedral: Dihedral angle around connecting bond (float, radian)
        dih_type: Definition type of dihedral angle (str; 'monomer' or 'bond')
        terminate_tail: If False, do not add/modify the tail terminator (bool)
    ...
    # ---- 头部处理 ----
    if Chem.MolToSmiles(mol1) in ('[H][3H]', '[3H][H]'):
        head_idx = poly.GetIntProp('head_idx')
        poly.GetAtomWithIdx(head_idx).SetIsotope(1)
        poly.GetAtomWithIdx(head_idx).GetPDBResidueInfo().SetResidueName(res_name_1)
        poly.GetAtomWithIdx(head_idx).GetPDBResidueInfo().SetResidueNumber(1 + poly.GetIntProp('num_units'))
    ...
    if terminate_tail:
        if mol2 is None:
            mol2 = mol1
        if Chem.MolToSmiles(mol2) in ('[H][3H]', '[3H][H]'):
            tail_idx = poly.GetIntProp('tail_idx')
            poly.GetAtomWithIdx(tail_idx).SetIsotope(1)
            poly.GetAtomWithIdx(tail_idx).GetPDBResidueInfo().SetResidueName(res_name_2)
            poly.GetAtomWithIdx(tail_idx).GetPDBResidueInfo().SetResidueNumber(1 + poly.GetIntProp('num_units'))
            poly.SetIntProp('num_units', 1 + poly.GetIntProp('num_units'))
        else:
            poly = connect_mols(
                poly, mol2,
...
def terminate_rw(poly, mol1, mol2=None, retry_step=100, dist_min=0.7,
    ...
    # Decide which ends to process
    if terminate_tail:
        mols = [mol1, mol2 if mol2 is not None else mol1]
    else:
        mols = [mol1]  # head only
    ...
    for i, mol in enumerate(mols):
        ...
        for r in range(retry_step):
            if i == 0:
                ...
                # Tail (only when terminate_tail=True, because mols 长度为 2)
                if H2_flag:
                    tail_idx = poly.GetIntProp('tail_idx')
                    poly.GetAtomWithIdx(tail_idx).SetIsotope(1)
                    poly.GetAtomWithIdx(tail_idx).GetPDBResidueInfo().SetResidueName(res_name_2)
